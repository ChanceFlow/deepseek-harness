...
本仓库的 `@deepseek-ai` 包发布在 Gitea 包仓库（`http://<gitea-legacy-host>:3000/api/packages/Chance/npm/`）。本文说明如何接入、如何选择与鉴别版本。
...
```ini
@deepseek-ai:registry=http://<gitea-legacy-host>:3000/api/packages/Chance/npm/
```
...
```ini
//<gitea-legacy-host>:3000/api/packages/Chance/npm/:_authToken=<token>
```
...
```sh
$ npm view @deepseek-ai/dsh dist-tags --registry=http://<gitea-legacy-host>:3000/api/packages/Chance/npm/
...
npm view @deepseek-ai/dsh@0.1.0-rc.8-chance.1 dist.integrity dist.tarball \
  --registry=http://<gitea-legacy-host>:3000/api/packages/Chance/npm/

# 浏览器查看全部版本与文件清单
http://<gitea-legacy-host>:3000/Chance/-/packages
```
