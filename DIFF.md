...
行为：全家族以 `<upstream 版本>-chance.N` 发布到本地 Gitea 私服（`http://<gitea-host>:3000/api/packages/ChanceFlow/npm/`，由 gitignore 的本地 `.npmrc` 指向）；发版时的版本 bump 与 lockfile 同时提交；lockfile 策略为"以 upstream 解析为基底 + fork 增量（pi-ai 钉版、undici）"，避免 dev 依赖漂移；`/dist/`、`/apps/cli/dist/` 为发版 staging 产物，已 gitignore；fork 自己不发 vendor 序列、不构建 `native/system` 序列、也不 vendor upstream 新钉的公共 `@deepseek-ai` 依赖——`pnpm run release:mirror-packages` 把 checkout 钉住的每个 `@deepseek-ai` 包从公共 registry 镜像进私服，钉住来源有三处：`vendor/*`（rescope 后的 Cordis，名称与版本跟 upstream 公共发布一致）、native 序列（`native/system/packages/*` 的版本）、以及 `pnpm-lock.yaml` 记录的 registry 解析（公共依赖，版本与 integrity 均以 lockfile 为准，workspace/link 解析不镜像）；三类都按"被依赖的包先于依赖它们的入口包"排序，tarball 字节必须与钉住来源记录的 integrity 一致，私服已有版本也必须记录同一 integrity。release workflow 在打包前自动执行，取代此前的手工镜像。
...
文件：`scripts/release/{registry,mirror-packages}.ts`、`scripts/release/mirror-packages.spec.ts`、`.gitea/workflows/release.yml`。
...
同步注意：版本号冲突（rc.2 实测 230 个 package.json）统一取 upstream，下一次 fork 发版再 `-chance` 化；registry 组织名必须以注册表规范大小写 `ChanceFlow` 书写（Gitea 路由不分大小写，但 pnpm 的 tarball 供应链校验区分大小写，lockfile/workflow 里的小写 `chanceflow` 会被 `[ERR_PNPM_TARBALL_URL_MISMATCH]` 拒绝）；升级生产 = `npm install -g @deepseek-ai/dsh && systemctl --user restart dsh`（见 `~/services/dsh/start.sh`）；镜像脚本按 `native/system/packages/*`（`prebuilds.json` 标记平台包）自动跟随 upstream 的改名与新增，按 `vendor/*` 跟随 rescope 后的 Cordis 版本，并按 lockfile 自动跟随 upstream 新钉的公共 `@deepseek-ai` 依赖；三者钉住的版本若 upstream 尚未发布，release 会在镜像步骤失败并报出版本号，此时应等 upstream 发布或用 `--source` 指定实际承载该版本的 registry。读取公共 registry 时必须同时覆盖 scope 路由：`@deepseek-ai:registry` 的优先级高于 `--registry`，只传 `--registry` 会让"源"读取落回私服、把私服与自己比对（`registrySourceArgs` 因此同时发出 `--registry` 与 `--<scope>:registry`）。0.1.5-rc.2 合入：272 个 `package.json` 的版本冲突统一取 upstream `0.1.5-rc.2`，无源码冲突；自动合并的 `pnpm-lock.yaml` 经 `pnpm install` 校验无需再对齐。0.1.7-rc.2 合入：273 个 `package.json` 的版本冲突统一取 upstream `0.1.7-rc.2`，另有 9 个包被 upstream 删除（`code-runtime-worker-thread`、`e2b`、`subprocess-e2b`、`experimental/agent-team-web-profile`、`experimental/code-runtime-python`、`fs/tool-present`、`preset/agent-presets`、`settings/settings-file`、`workflow/workflow-worker-thread`），fork 侧仅改过版本号，因此接受删除；`pnpm-lock.yaml` 取 upstream 解析后 `pnpm install` 无差异；根 `package.json` 的镜像脚本行在取 upstream 后需重新加回。同一窗口 upstream 新引入公共依赖 `@deepseek-ai/libreoffice-kit@0.1.1`（`packages/document/office-to-pdf`、`packages/skill/skill-office`、`packages/bundle/web-app`、`apps/desktop-host` 依赖它），私服原本没有该包，`pnpm install` 会以 404 失败；已把镜像步骤从"只镜像 native 序列"推广为"镜像钉住来源的全部 `@deepseek-ai` 包"（脚本与 `release:*` 名字随之改为 `mirror-packages`），lockfile 里那 6 个 `libreoffice-kit` 包由该步骤覆盖，不再需要手工镜像；`registrySourceArgs` 同时修掉了只传 `--registry` 时的自比对。注意 `pnpm install --frozen-lockfile` 在 CI 里先于镜像步骤运行，因此 upstream 首次钉入一个私服没有的新包时仍要人工先跑一次镜像步骤。同一窗口 upstream 把 vendored Cordis 从 4.0.2 提到 4.0.4，而 vendor 序列 fork 自己从不发布：0.1.7-rc.2-chance.0 首次发布后，任何 `npm install -g @deepseek-ai/dsh@0.1.7-rc.2-chance.0` 都会以 `ETARGET @deepseek-ai/cordis@~4.0.4` 失败（QS Mac 实测），把 `vendor/*` 纳入镜像来源后 9 个 vendor 包补进私服即可安装。**发版后必须用一次真实安装验证**：`release:verify-packed-install` 与 CI 都不会发现缺 vendor/native 版本，只有消费者安装才会。
...
同步注意：preset 行里的插件用**包子路径**引用（`@deepseek-ai/dsh-web-app/presets/plugins/<name>.js`），不能用相对路径——entry 名称一律相对 profile 目录解析，相对路径会让每台机器都缺 `~/.dsh/profiles/web/plugins/` 而报 `never started`；包子路径经由 profile 的 node_modules 拦截层解析到已安装的 bundle，因此 `exports` 必须保留 `./presets/plugins/*`。另外 profile 的 node_modules 拦截层是指向"当前安装"的软链集合，dsh 换安装位置（例如从 `/usr/local` 换到 `~/.npm-global`）后它不会自动重指，profile 层引用新包会 `ERR_PACKAGE_PATH_NOT_EXPORTED`/`Cannot find package`。声明式 preset **不合并** builtin 变更，每行自带完整子插件列表，因此 upstream 改动 standard 组合（增删行、改 id）时，这三个文件里复制的列表必须同步更新，否则 preset 与标准组合漂移。`godot` preset 依赖 `~/.dsh/godot-backends.yml` 与 `white-win` 这个 SSH 别名（只在 nuc 的 `~/.ssh/config` 里定义），别的机器上该 preset 会持续刷 `Could not resolve hostname white-win`，要么补别名、要么在那些机器上不选它。
