...
行为：全家族以 `<upstream 版本>-chance.N` 发布到本地 Gitea 私服（`http://<gitea-host>:3000/api/packages/ChanceFlow/npm/`，由 gitignore 的本地 `.npmrc` 指向）；发版时的版本 bump 与 lockfile 同时提交；lockfile 策略为"以 upstream 解析为基底 + fork 增量（pi-ai 钉版、undici）"，避免 dev 依赖漂移；`/dist/`、`/apps/cli/dist/` 为发版 staging 产物，已 gitignore；fork 既不构建 `native/system` 序列，也不 vendor upstream 新钉的公共 `@deepseek-ai` 依赖——`pnpm run release:mirror-packages` 把 checkout 钉住的每个 `@deepseek-ai` 包从公共 registry 镜像进私服，钉住来源有两处：native 序列取 `native/system/packages/*` 的版本，公共依赖取 `pnpm-lock.yaml` 记录的 registry 解析（版本与 integrity 均以 lockfile 为准，workspace/link 解析不镜像）；两类都按"被依赖的包先于依赖它们的入口包"排序，tarball 字节必须与钉住来源记录的 integrity 一致，私服已有版本也必须记录同一 integrity。release workflow 在打包前自动执行，取代此前的手工镜像。
