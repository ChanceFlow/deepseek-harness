...
GitHub now receives every fork master commit within one Actions queue delay, with no second place to publish. The GitHub-side repository is a public fork of upstream `deepseek-ai/deepseek-harness` whose `master` mirrors the Gitea master, and its Actions are disabled so upstream `.github/workflows` never execute there. A visibility conversion on GitHub detaches a repository from its parent fork network irreversibly; restoring the fork relationship means deleting the repository and re-forking from the parent, which is safe for this mirror only because its sanitized content regenerates deterministically from Gitea — visibility flips must therefore never be performed on the fork. The mirror's correctness rests on facts outside this file: the `dsh-host` runner staying registered with `gh` logged in as `ChanceFlow`, the filter-repo expression list staying current with whatever internal identifiers new work introduces (a new LAN host, path, or token-shaped string added to Gitea history is public on GitHub until added to the list — the leak-prevention duty therefore falls on each change, not on the mirror), and the personal author email no longer appearing in new commit metadata (this repository's git identity is set to the `ChanceFlow` noreply address; the sync-upstream cron's merge identity was switched with it). Anyone with GitHub write access who pushes to the fork's master will have it overwritten by the next Gitea push, which is the intended authority order. Force-push semantics mean a local correction pushed to Gitea instantly rewrites the fork; the fork never diverges.
