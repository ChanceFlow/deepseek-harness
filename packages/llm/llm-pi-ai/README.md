...
Successful assistant responses store a versioned, lossless-JSON replay state beside the provider and model that produced them — response-level facts plus one per-block entry per streamed block. At request time, `LlmRuntime` passes replay state only when the same adapter instance owns both routes; the adapter validates it and restores native response ids, provider signatures, and optional `providerThinkingLevel` effort metadata, keeping absent effort metadata absent. Replay validates the requested model identity against the assistant source and keeps a provider-reported alias or fallback in `responseModel`; reconstruction restores the requested model as the message identity, which is what pi-ai compares to decide same-model continuation. Absent or unusable replay state degrades to provider-neutral content while preserving the assistant source’s required provider and model. On a route that declares `compat.allowEmptySignature`, a replayed assistant turn containing a tool call holds a thinking block: a turn whose provider recorded no reasoning gains an empty one with a placeholder signature, because such an endpoint rejects the request otherwise while pi-ai drops an empty-text block that carries no signature. Provider-neutral history is left as the provider recorded it. pi-ai tool-call arguments are parsed objects, so the adapter parses input and re-stringifies output to the harness raw-JSON convention; pi-ai in-stream error events map to terminal `finish` chunks.
